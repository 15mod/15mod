...
_FA18 animate ["ejection_seat_wso_hide",0];
...
if (isPipEnabled) then {_FA18 animate ["Mirrors_pip_hide",0]; _FA18 animate ["WSO_pip_hide",0];} else {_FA18 animate ["Mirrors_pip_hide",1]; _FA18 animate ["WSO_pip_hide",1];};
...
If ((damage _FA18 > 0.85) and (player != driver _FA18) and (player != gunner _FA18)) then {[_FA18] spawn JS_JC_fnc_FA18_Ejection_F;};
