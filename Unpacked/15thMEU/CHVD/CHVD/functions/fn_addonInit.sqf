[] spawn {
	...
	// 15thMEU EDIT for ACE3
	if !(isClass (configFile >> "CfgPatches" >> "meu_blufor") && isClass (configFile >> "CfgPatches" >> "ace_main")) then { //<--
		_actionText = if (isLocalized "STR_chvd_title") then {localize "STR_chvd_title"} else {"View Distance Settings"};
		player addAction [_actionText, CHVDAddon_fnc_openDialog, [], -99, false, true];
		player addEventHandler ["Respawn", format ["player addAction ['%1', CHVDAddon_fnc_openDialog, [], -99, false, true]", _actionText]];
	}; //<--
	
	//Detect when to change setting type
	[] spawn {
		for "_i" from 0 to 1 step 0 do {
			[nil, false] call CHVDAddon_fnc_updateSettings;
	...
	[] spawn {
		for "_i" from 0 to 1 step 0 do {
			...
			[nil, true] call CHVDAddon_fnc_updateSettings;
			waitUntil {UAVControl (getConnectedUAV player) select 1 == ""};
			[nil, false] call CHVDAddon_fnc_updateSettings;
		};
	};
